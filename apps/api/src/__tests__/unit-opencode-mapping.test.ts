...
describe('ensureOpencodeSessionPin', () => {
  ...
  test('uses daemon health for a non-OpenCode ACP runtime', async () => {
    globalThis.fetch = (async (url: RequestInfo | URL, init?: RequestInit) => {
      fetchCalls.push({ method: init?.method ?? 'GET', url: String(url) });
      if (String(url).endsWith('/session?directory=%2Fworkspace')) {
        return new Response(JSON.stringify({ error: 'runtime REST API is not available' }), {
          status: 404,
          headers: { 'Content-Type': 'application/json' },
        });
      }
      return new Response(
        JSON.stringify({
          ...
          opencode_session_id: 'ses_claude',
        }),
        {
          status: 200,
          headers: { 'Content-Type': 'application/json' },
    ...
    expect(fetchCalls.map((call) => call.url)).toEqual([
      'https://sandbox.test/session?directory=%2Fworkspace',
      'https://sandbox.test/kortix/health',
    ]);
    expect(dbUpdates).toHaveLength(0);
  });

  test('waits when a non-OpenCode ACP runtime has no canonical session', async () => {
    globalThis.fetch = (async (url: RequestInfo | URL, init?: RequestInit) => {
      fetchCalls.push({ method: init?.method ?? 'GET', url: String(url) });
