...
import { deriveInflightBudgetBytes, detectContainerMemoryLimitBytes } from './container-memory';
...
export function mountLlmGateway(app: OpenAPIHono): void {
  if (!config.LLM_GATEWAY_ENABLED) {
    ...
    // Derived from THIS container's real memory limit unless explicitly
    // overridden. No constant can be right in both a 640m self-host container
    // and a 4 GiB ECS task, and a self-host compose file is rendered once and
    // never regenerated by an update — so the process reads reality rather than
    // trusting a file somebody forgot to re-render. See container-memory.ts.
    const containerLimitBytes = detectContainerMemoryLimitBytes();
    const budgetBytes =
      config.GATEWAY_INFLIGHT_BUDGET_BYTES > 0
        ? config.GATEWAY_INFLIGHT_BUDGET_BYTES
        : deriveInflightBudgetBytes(containerLimitBytes);
    const asMiB = (n: number): string => `${Math.round(n / (1024 * 1024))} MiB`;
    // Logged because an operator debugging a 503 needs the number the process
    // actually chose, not the one they assume it derived.
    console.info(
      `[gateway] in-flight budget ${asMiB(budgetBytes)} ` +
        `(container limit ${containerLimitBytes === null ? 'unknown' : asMiB(containerLimitBytes)}, ` +
        `${config.GATEWAY_INFLIGHT_BUDGET_BYTES > 0 ? 'explicitly overridden' : 'derived'})`,
    );
