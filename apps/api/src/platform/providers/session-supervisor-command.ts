...
export function buildSessionSupervisorCommand(
  ...
  const stopOldProcesses = String.raw`for proc in /proc/[0-9]*; do [ -r "$proc/cmdline" ] || continue; pid=${'${proc#/proc/}'}; [ "$pid" = 1 ] && continue; cmd=$(tr '\0' ' ' < "$proc/cmdline" 2>/dev/null); cmd=${'${cmd% }'}; case "$cmd" in "/opt/kortix/agent.current run"|"/opt/kortix/agent.bootstrap run"|"/opt/kortix/agent.bootstrap supervise"|"/opt/kortix/node/runtime/agent.current run") kill -TERM "$pid" 2>/dev/null || true;; esac; done; sleep 1`;
  if (!identity) {
  ...
  return `install -d -o kortix -g kortix -m 0755 /opt/kortix /opt/kortix/node; if ! mkdir /opt/kortix/bootstrap.lock 2>/dev/null; then exit 0; fi; trap "rmdir /opt/kortix/bootstrap.lock 2>/dev/null || true" EXIT HUP INT TERM; ${stopOldProcesses}; tmp=/opt/kortix/agent.bootstrap.tmp.$$; curl -fsSL --retry 3 --retry-delay 1 -H ${quoteShell(`Authorization: Bearer ${identity.nodeToken}`)} ${quoteShell(assetUrl)} -o "$tmp"; chmod 0755 "$tmp"; "$tmp" version >/dev/null; mv -f "$tmp" /opt/kortix/agent.bootstrap; chown kortix:kortix /opt/kortix/agent.bootstrap; ${identityEnv.trim()} KORTIX_API_URL=${quoteShell(normalizedRelayUrl)} KORTIXD_HOME=/opt/kortix/node KORTIXD_ADOPT_EXISTING_RUNTIME=1 HOME=${quoteShell('/home/kortix')} USER=kortix LOGNAME=kortix PATH=${quoteShell(runtimePath)} setsid -f setpriv --reuid kortix --regid kortix --init-groups /opt/kortix/agent.bootstrap supervise >>/tmp/kortix-entrypoint.log 2>&1 </dev/null`;
}
