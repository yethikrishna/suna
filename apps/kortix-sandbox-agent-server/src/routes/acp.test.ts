...
describe('authenticated OpenCode ACP bridge', () => {
  ...
  test('maps Pi session/resume to the adapter session/load method', async () => {
    const seen: JsonRpcEnvelope[] = []
    const app = createAcpRouter(
      config(),
      () =>
  ...
  test('resolves the runtime harness for each request after warm adoption', async () => {
    const seen: JsonRpcEnvelope[] = []
    let runtimeHarness: 'opencode' | 'pi' = 'opencode'
    const app = createAcpRouter(
      config(),
      () =>
        connection({
    ...
    expect((await requestResume()).status).toBe(202)
    runtimeHarness = 'pi'
    expect((await requestResume()).status).toBe(202)
    expect(seen.map((envelope) => envelope.method)).toEqual([
      'session/resume',
      'session/load',
    ])
  })
