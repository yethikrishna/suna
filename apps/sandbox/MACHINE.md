...
Python 3 is available as `python` and `python3`. Pillow is pre-installed (`from PIL import Image` works bare).
Use `uv run --with "pkg1,pkg2" script.py` for all other Python dependencies — declare them inline, comma-separated, version pins allowed. Do not create venvs or use `pip install` for one-off scripts; assume no Python packages besides Pillow are pre-installed.
