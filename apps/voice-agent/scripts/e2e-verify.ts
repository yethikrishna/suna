...
async function main() {
  ...
  // Reuse the bridge token the runtime already minted (it carries roomJoin +
  // canPublish + canSubscribe) rather than depending on livekit-server-sdk,
  // which lives in apps/api and is not a dependency of this workspace.
  const token = arg('token');
  if (!token) throw new Error('--token <livekit jwt> required');
