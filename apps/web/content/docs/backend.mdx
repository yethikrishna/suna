...
The public field is `opencode_model` for API and SDK compatibility. Do not rename
this wire field.
...
The example below uses the OpenCode REST compatibility stream.
...
In React, `useSession()` owns the whole session lifecycle — start, sandbox
readiness, the live stream, and message sync. Prefer it over the escape hatches
above.
...
| `agent_name` | **No** — a session keeps the agent it started with. |
