...
| Field                | Contract                                                                                                                                                |
| -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `agent_name`         | Selects a declared logical agent. A v3 agent also selects its immutable runtime profile and harness.                                                    |
| `opencode_model`     | Selects the initial model. The wire name remains stable for every supported harness. An unavailable model returns `400 INVALID_SESSION_MODEL`.          |
...
| `require_connectors` | Adds mandatory connector profiles for this create request. Missing authorizations return `409 CONNECTOR_AUTHORIZATION_REQUIRED` before sandbox startup. |
...
Await runtime readiness before using the OpenCode REST compatibility methods:
...
| `409`                        | `CONNECTOR_AUTHORIZATION_REQUIRED`               | A mandatory connector profile has no valid active authorization.                   |
