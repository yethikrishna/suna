...
| `agent_name` | Which declared OpenCode agent handles the session. |
| `runtime_context` | Arbitrary non-secret key/values passed to the run as context. |

The public field remains `opencode_model` for API and SDK compatibility. It
sets the initial OpenCode model.
...
The example below uses the OpenCode REST stream.
