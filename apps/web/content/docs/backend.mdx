...
| `secrets`, `connector_bindings` | **Yes** — `PUT /projects/{id}/sessions/{sid}/scope`. SET semantics: what you send replaces the current list, from the next prompt. Dropping a secret stops **delivery** — it cannot un-read what the agent already has, so rotate if you need real revocation. Bindings resolve at call time and so ARE fully retroactive. |
| `runtime_context`, `end_user_ref` | **No** — set once at create |
