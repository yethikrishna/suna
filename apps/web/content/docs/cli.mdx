...
general-purpose starter. It includes OpenCode, Claude Code, Codex, and Pi
runtime profiles.
...
starter writes a v3 `kortix.yaml` with OpenCode, Claude Code, Codex, and Pi
runtime profiles. It writes the canonical `.kortix/opencode` system-skill
source. The command can wire OpenCode, Claude Code, Codex, Pi, and Cursor for
local discovery. Claude Code and Pi keep their real runtime directories. The
CLI adds native discovery links inside them. The command does not write
...
The local coding-tool selection does not select a cloud session harness.
Cloud project creation enables **ACP & Multi-Harness** for the starter. An
existing project keeps its current experiment state.
...
| `kortix executor call <connector>.<action> '<json-args>'` | Run a tool. The legacy split form remains supported. |
