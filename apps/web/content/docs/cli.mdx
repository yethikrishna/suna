...
`kortix init [project-name] [options]` creates a new project directory. It
writes a v2 `kortix.yaml`, `.kortix/Dockerfile`, and the canonical
`.kortix/opencode` system-skill source. It can wire OpenCode, Claude Code,
Codex, Pi, and Cursor for local discovery. It does not write
`.kortix/link.json` — `kortix ship` or `kortix projects link` create that later.
...
The local coding-tool selection does not select a cloud session harness. Enable
the **ACP & Multi-Harness** project experiment and use a v3 manifest runtime
profile for cloud OpenCode, Claude Code, Codex, or Pi selection.
