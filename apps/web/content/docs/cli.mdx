...
This command attaches to the OpenCode REST compatibility process.
...
starter writes a `kortix_version: 2` `kortix.yaml` and the canonical
`.kortix/opencode` system-skill source.

`--primary` and `--agents` wire the coding tools **you run on your own machine**
— OpenCode, Claude Code, Codex, Pi, or Cursor — so each one discovers the
starter's skills natively. This changes nothing about the cloud runtime: every
cloud session runs OpenCode. The command does not write `.kortix/link.json`.
`kortix ship` or `kortix projects link` create that file.
...
`--agents` wires up the coding agents you run on your own machine. It does not
change what a cloud session runs; every cloud session runs OpenCode.
...
sessions, sandboxes, the executor, memory, and channels. The API serves them
live, so they match the deployed host version.
