...
`kortix init` creates a project directory with the
`general-knowledge-worker` template by default. Use
`--template acp-multi-harness` to scaffold OpenCode, Claude Code, Codex, and
Pi runtime profiles.
...
This command attaches to the OpenCode REST compatibility process. It does not
attach a local Claude Code, Codex, or Pi client to the selected ACP process.
...
default template writes a v2 `kortix.yaml`. The `acp-multi-harness` template
writes a v3 `kortix.yaml` with OpenCode, Claude Code, Codex, and Pi runtime
profiles. Both templates write `.kortix/Dockerfile` and the canonical
`.kortix/opencode` system-skill source. The command can wire OpenCode, Claude
Code, Codex, Pi, and Cursor for local discovery. It does not write
...
| `--template <general-knowledge-worker\|acp-multi-harness\|minimal>` | Project template. Default: `general-knowledge-worker`. `acp-multi-harness` declares all four ACP harnesses. |
| `--force` | Configure the current directory in place instead of scaffolding a new one. |
| `--overwrite` | Overwrite existing files. |
| `--no-git` | Skip git init. |
| `-y, --yes` | Don't prompt. |

The local coding-tool selection does not select a cloud session harness. The
`acp-multi-harness` starter selects a v3 manifest and enables the **ACP &
Multi-Harness** project experiment when the cloud project is created. For an
existing project, enable that experiment before starting a v3 session.
...
| `kortix sessions connect [<id>] [-- <opencode args>]` | Attach your local OpenCode TUI to the OpenCode REST compatibility process in the sandbox. It does not attach to Claude Code, Codex, or Pi. Alias: `attach`. |
...
sessions, sandboxes, ACP, all four runtime harnesses, the executor, memory, and
