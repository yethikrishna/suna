...
Sessions a trigger creates are private by default. The trigger agent's service
account owns them. The person who configured or manually fired the trigger does
not gain access through that action.

Trigger settings offer three policies:

- **Only the trigger agent** — no teammate can open a trigger-created session.
- **Selected teammates** — the trigger agent plus selected project members and account groups.
