...
Self-hosted instances do not start a warm session when a user opens a project.
This setting protects fixed sandbox capacity. Enable warm sessions with:

```sh
kortix self-host env set KORTIX_PUBLIC_WARM_PROJECT_SESSIONS_ENABLED=true
```
