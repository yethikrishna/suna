...
Kortix still fully supports v1 manifests. The platform resolves a project's manifest in this order: `kortix.yaml`, then `kortix.yml`, then `kortix.toml`. The default general-purpose starter creates a v2 `kortix.yaml`. The selectable ACP multi-harness starter creates a v3 `kortix.yaml`. A v1 project keeps working with no forced upgrade.
