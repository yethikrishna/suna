...
The manifest is the file the platform treats as authoritative for a project. This page lists the `kortix.yaml` v2 fields. For the plain-language version, see [Your project](/docs/project).

## Version

`kortix_version: 2` is the current schema. Every new project scaffolds it, and it runs OpenCode. `kortix.toml` v1 still works for existing projects with no forced upgrade.

<Callout type="warn" title="There is no v3">
A `kortix_version: 3` schema exists in the codebase. It is **experimental,
unreleased, and off by default** — you cannot create or run a v3 project. Write
`kortix_version: 2`.
</Callout>
...
- **OpenCode config** — `.kortix/opencode/`. OpenCode reads its own directory.

The join between the two halves is the agent name. A manifest key `agents.<name>` must match the filename `.kortix/opencode/agents/<name>.md`. Every behavioral field lives only in that `.md` file's frontmatter and body:
...
Any repo with a valid manifest at its root is a Kortix project. `kortix.yaml` v2 is YAML-only. `kortix.toml` v1 still works for existing projects. See [Legacy TOML](/docs/project/legacy-toml) for the v1 schema.

`kortix_version: 2` requires YAML. A TOML file that declares it fails validation.
...
- A manifest or `.kortix/opencode/` config edit applies only after a change request merges to `main`. Sessions and the trigger sweep read the default branch, not session branches.
