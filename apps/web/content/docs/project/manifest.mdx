...
description: Every kortix.yaml v2 and v3 key, with defaults, limits, and validation rules.
---

The manifest is the file the platform treats as authoritative for a project. This page lists the `kortix.yaml` v2 and v3 runtime fields. It also lists the shared project fields. For the plain-language version, see [Your project](/docs/project).

## Choose v2 or v3

- Use `kortix_version: 3` for new projects and multiple harnesses.
- Keep `kortix_version: 2` for an existing OpenCode REST project.

Version 3 supports `opencode`, `claude`, `codex`, and `pi`. New generic
projects enable **ACP & Multi-Harness** automatically. Enable the experiment
before creating v3 sessions in an existing project. A disabled project uses
the OpenCode REST transport.

The runtime choice is immutable per session. The selected logical agent fixes the runtime profile, harness, and optional harness-native agent.
...
### Version 3 runtime fields

| Field | Required | Notes |
|---|---|---|
| `kortix_version` | yes | Must be `3`. Version 3 is YAML-only. |
| `default_agent` | yes | Must name a declared, enabled logical agent. |
| `runtimes` | yes | Non-empty map of immutable runtime profiles. |
| `runtimes.<name>.harness` | yes | `opencode`, `claude`, `codex`, or `pi`. |
| `runtimes.<name>.config_dir` | no | Repo-relative path. The harness default applies when omitted. |
| `agents` | yes | Non-empty map of logical agents. |
| `agents.<name>.runtime` | yes | Must name a profile in `runtimes`. |
| `agents.<name>.agent` | no | Harness-native agent or mode. Kortix stores it as `native_agent`. |
| `agents.<name>.enabled` | no | Defaults to `true`. |
| `agents.<name>.connectors` | no | Connector grants. Defaults to `none`. |
| `agents.<name>.secrets` | no | Secret grants. Defaults to `none`. |
| `agents.<name>.skills` | no | Skill grants. Defaults to `none`. |
| `agents.<name>.kortix_cli` | no | Kortix CLI grants. Defaults to `none`. |
| `agents.<name>.workspace` | no | `runtime`, `read`, or `branch`. |

Version 3 rejects OpenCode behavior fields such as `model`, `mode`, and `permission` in a logical-agent block. Put native behavior in the selected harness configuration directory.
...
- **Harness config** — `.kortix/opencode/`, `.claude/`, `.codex/`, or `.pi/`. The selected harness reads its own directory.

For v2, the join between the two halves is the agent name. A manifest key `agents.<name>` must match the filename `.kortix/opencode/agents/<name>.md`. Every behavioral field lives only in that `.md` file's frontmatter and body:
...
Any repo with a valid manifest at its root is a Kortix project. `kortix.yaml` v2 and v3 are YAML-only. `kortix.toml` v1 still works for existing projects. See [Legacy TOML](/docs/project/legacy-toml) for the v1 schema.

`kortix_version: 2` and `kortix_version: 3` require YAML. A TOML file that declares either version fails validation. A manifest that declares a version higher than `3` is rejected.
...
- [`kortix.v3.schema.json`](/schema/kortix.v3.schema.json)
...
| `authorization_strategy` | no | `project` or `user`. Defaults to `project` when omitted. A project strategy uses active project authorizations. A user strategy uses only the acting member's authorization. |
...
belong to the connector profile. Every authorization under that connector
profile uses the same policies.
...
| `connectors_required` | `none` | Connector-profile slugs that must resolve an active, strategy-compatible authorization before sandbox startup. Must be a subset of `connectors`. A missing authorization returns `CONNECTOR_AUTHORIZATION_REQUIRED`. |
...
connector profile's `authorization_strategy` decides whether the authorization
...
- A manifest or harness-native config edit applies only after a change request merges to `main`. This includes `.kortix/opencode/`, `.claude/`, `.codex/`, and `.pi/`. Sessions and the trigger sweep read the default branch, not session branches.
