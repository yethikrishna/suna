...
- Use `kortix_version: 2` for one OpenCode runtime.
- Use `kortix_version: 3` for ACP runtime profiles and multiple harnesses.

Version 3 supports `opencode`, `claude`, `codex`, and `pi`. Enable the project experiment named **ACP & Multi-Harness** before creating v3 sessions. A disabled project uses the OpenCode REST transport.
