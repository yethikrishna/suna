...
experimental [feature flag](/docs/feature-flags), and it is **off by
default** — turn it on in Settings → Experimental (operators can default a
whole deployment on with `LLM_GATEWAY_DEFAULT_ENABLED`).

With the flag off, the project runs **native OpenCode model management**:
