...
restarts the OpenCode compatibility process. Restart an active Claude Code,
Codex, or Pi session after rotating a direct harness credential.
</Step>
...
3. If the changed secret is an LLM provider credential, Kortix restarts the
   OpenCode compatibility process. An already-running Claude Code, Codex, or Pi
   ACP process retains its launch environment.
...
Direct credential names differ by runtime harness.

| Harness | Direct credentials |
|---|---|
| OpenCode | Provider variables referenced by the OpenCode provider config. |
| Claude Code | `ANTHROPIC_API_KEY`, `ANTHROPIC_AUTH_TOKEN`, or `CLAUDE_CODE_OAUTH_TOKEN`. |
| Codex | `CODEX_API_KEY` or `OPENAI_API_KEY`. Kortix manages ChatGPT OAuth state. |
...
prompt through the exact harness and model.
