...
|---|---|---|
| **Egress-enforced** | A **handle** — a self-describing placeholder, worth nothing on its own | Everything the agent sends over HTTPS. This is the default. |
| **Environment** | The real value, as a plain environment variable | Values the agent must compute with, and protocols that are not HTTPS |
...
Use **egress-enforced** as much as possible, and **environment** as little as
possible. Environment exposure puts a real credential in a place the agent can
read, print, and forward. Every other exposure keeps it outside.
</Callout>
...
- **Sent secrets** — the value travels on the wire. API keys, bearer tokens,
  ...
  sandbox. Sent secrets belong on **egress-enforced**.
...
That is the whole reason environment exposure still exists. It is not a
weaker default; it is the only exposure that can serve a credential the
sandbox has to do math with. Non-HTTPS protocols — a Postgres connection
string, SMTP credentials — are in the same position.
...
Choose it only when the credential is computed rather than sent, or when the
protocol is not HTTPS. Everything else belongs on egress-enforced exposure.
...
encrypts the value immediately and picks an exposure for you: a recognized
model key gets its vendor host prefilled, recognizable signing material gets
**environment**, and everything else gets **egress-enforced** with an empty
host list for you to fill.

One control changes it: **"Can your code read this value?"** Answering *no*
keeps egress enforcement and shows the host list. Answering *yes* moves the
secret to environment exposure, with a warning.
</Step>
...
| `kortix secrets delivery IDENTIFIER egress --allow-host <host>` | Egress-enforced exposure for the listed hosts |
| `kortix secrets delivery IDENTIFIER runtime` | Environment exposure |
| `kortix secrets delivery IDENTIFIER denied` | Disabled |
| `kortix secrets call IDENTIFIER URL` | Send one policy-bound HTTPS request through Kortix |
