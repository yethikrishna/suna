...
This page shows the OpenCode REST compatibility SDK path in one file. It uses
no framework and needs no build step beyond TypeScript. Use
[`useSession`](/docs/sdk/react) for a React surface that owns the whole session
lifecycle.
