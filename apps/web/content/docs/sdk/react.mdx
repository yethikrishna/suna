...
| `runtimeHarness` | `'opencode' \| 'claude' \| 'codex' \| 'pi' \| null` | Immutable session harness. Display it for diagnostics only. |
| `nativeAgent` | `string \| null` | Immutable harness-native agent or mode. |
| `acpServerId`, `acpSessionId` | `string \| null` | ACP process key and harness-native conversation id. |
...
The project experiment `acp_runtime` selects ACP. The default is REST.
`useSession` owns both paths. A v3 session can select OpenCode, Claude Code,
Codex, or Pi. The host does not construct `/kortix/acp/` URLs, call OpenCode
prompt routes directly, or branch on `runtimeHarness`. Disable the experiment
to use OpenCode REST for compatible projects.
