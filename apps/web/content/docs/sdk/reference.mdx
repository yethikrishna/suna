...
`runtime` with consumer `sandbox` is **environment** exposure — the only policy
that puts a plaintext value in the session. `egress` with consumer `network` is
**egress-enforced** exposure: the session holds a handle and Kortix substitutes
the real value outside the sandbox, for the exact HTTPS hosts the policy lists.
Every `broker` consumer has no session presence at all. See
