...
kortix.runtime(); // OpenCode REST compatibility client — not harness-neutral
```
...
`kortix.runtime()` returns the typed OpenCode REST compatibility client for the
active sandbox. It is not a harness-neutral ACP client.
...
| ACP | Harness-neutral client, projection, and session controller for OpenCode, Claude Code, Codex, and Pi |
