...
| `s.scope()` | `GET …/sessions/:sid/scope` | Reads stored secret narrowing and materialized connector authorizations |
...
Read the stored secret narrowing and materialized connector authorizations.
...
await s.rescope({
  ...
  connector_bindings: {
    github: { authorization_id: authorizationId },
  },
});
```

Each supplied field replaces its complete previous value. Omit a field to leave
it unchanged. Connector authorization changes apply to the next tool call.
...
For ACP sessions, use `useSession()` in React. It selects ACP from
`POST /start` and supports OpenCode, Claude Code, Codex, and Pi through one
projected message model. A host must not branch on `runtime_harness`.
...
`s.runtime` is the typed OpenCode REST compatibility client. It is not the
selected ACP harness client. Use it only for OpenCode REST calls that `send`,
...
For an ACP session, use `useSession()` in React. Framework-free hosts can use
`createAcpSessionController()` with `buildProjectAcpEndpoint()`. Both surfaces
support OpenCode, Claude Code, Codex, and Pi without host-side harness branches.
