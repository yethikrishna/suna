...
Two related gates are separate on purpose:

- `KORTIX_ENABLED_HARNESSES` decides **which** harness a manifest may launch (default: OpenCode only). ACP on with an empty allowlist still runs OpenCode alone. See [the manifest](/docs/project/manifest).
- A `kortix_version: 3` manifest has no REST path. Creating a session in a v3 project without the ACP experiment returns `409 ACP_RUNTIME_REQUIRED` instead of silently running REST against a v3 runtime plan. Projects scaffolded from the experimental multi-harness starter are therefore created with the experiment already on.
