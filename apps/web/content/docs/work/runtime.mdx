...
Kortix injects these variables at boot, alongside your project's [secrets](/docs/project/secrets). A secret arrives as an environment variable holding either a handle (egress-enforced exposure, the default) or the real value (environment exposure).
