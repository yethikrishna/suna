...
A session runs in an isolated sandbox on Daytona, Platinum, or E2B Cloud. Every project runs OpenCode over its REST compatibility interface.
...
Kortix runs on **OpenCode REST**. The daemon fronts OpenCode's REST
compatibility interface on `127.0.0.1:4096`. Every session launches one
`opencode` process. There is nothing to choose and nothing to configure.

A session binds its runtime at create time and never changes it.

<Callout type="warn" title="ACP and multi-harness are not available">
An unreleased ACP transport and a multi-harness runtime (`kortix_version: 3`,
Claude Code / Codex / Pi) exist in the codebase behind
`KORTIX_ACP_RUNTIME`, which defaults to `false`. They are **experimental,
unfinished, and unsupported**. Do not build on them, and do not enable the flag
on a deployment that serves real work. The rest of these docs describe OpenCode
REST, which is the only supported runtime.
</Callout>

## The daemon control surface

The `kortix-agent` binary runs as PID 1's child. It fronts the OpenCode REST
compatibility process on `KORTIX_SERVICE_PORT` (`8000`). Every route outside
`/kortix/*` requires the HMAC-signed `X-Kortix-User-Context` header, validated
against `KORTIX_SANDBOX_TOKEN`.

| Path | Purpose |
| --- | --- |
| `GET /kortix/health` | Liveness check. Reports runtime, repo, branch, and commit. |
...
```
┌─────────────────────────────────────────┐
│  Kortix runtime layer (added on top)    │  ← OpenCode + kortix-agent
├─────────────────────────────────────────┤
...
- OpenCode, the agent runtime every session boots.
