...
Kortix ships on **OpenCode REST**. A project without the **ACP & Multi-Harness** experiment talks to OpenCode over its REST compatibility interface on `127.0.0.1:4096`.

A project with the experiment enabled uses ACP. The daemon starts the selected harness from the immutable runtime plan. The supported harnesses are OpenCode, Claude Code, Codex, and Pi.

The choice is a client transport, not a process. Every session launches one `opencode acp` process, and both transports reach that same process. Switching a project does not change what runs in the sandbox.

### Turning ACP on

ACP is experimental. One environment variable holds the whole switch.

| Variable | Default | Effect |
|---|---|---|
| `KORTIX_ACP_RUNTIME` | `false` | The fleet default for the **ACP & Multi-Harness** experiment. `false` puts every project with no explicit choice on OpenCode REST. `true` puts them on ACP. |

Two ways to enable it:

- **A whole deployment.** Set `KORTIX_ACP_RUNTIME=true` and restart the API. Every project that has not chosen for itself moves to ACP.
- **One project.** Turn on **ACP & Multi-Harness** in Customize → Settings → Experimental. The project's own choice always wins over the fleet default, in both directions: a project stays on ACP with the variable off, and stays on REST with the variable on.

A session binds its transport at create time and never changes it. Flipping either switch affects the next session, never a running one.

### What `kortix_version: 3` depends on

`kortix_version: 2` is the stable manifest. It runs OpenCode, over REST or ACP. `kortix_version: 3` adds multi-harness runtime profiles, is unfinished, and runs **only** over ACP. So v3 follows the same switch:

- New projects scaffold `kortix_version: 2`. That is the default starter, and it does not change with the flag.
- With `KORTIX_ACP_RUNTIME=false`, asking to scaffold the experimental multi-harness starter returns `409 ACP_RUNTIME_DISABLED`. The refusal happens before any repository is created — Kortix does not hand you a project that could never start a session, and does not silently substitute the v2 starter you did not ask for.
- Creating a session in a project whose repository already declares `kortix_version: 3`, without the ACP experiment, returns `409 ACP_RUNTIME_REQUIRED`. Turn the experiment on for that project to run it.
- `KORTIX_ENABLED_HARNESSES` is a separate gate: it decides **which** harness a v3 manifest may launch (default: OpenCode only). ACP on with an empty allowlist still runs OpenCode alone. See [the manifest](/docs/project/manifest).
