...
/usr/local/bin/kortixd           ← The standalone node daemon.
...
Kortix injects these variables at boot, alongside your project's [secrets](/docs/project/secrets). A secret arrives as an environment variable holding the real value (environment exposure, the default) or, when the experimental `secrets_egress` flag is on, a handle (egress-enforced exposure).
...
`KORTIX_SANDBOX_TOKEN` is not a project API token. The `kortix` CLI does not read it, and project-scoped API routes reject it. Use `KORTIX_CLI_TOKEN` for any call that should act as the launching user.
