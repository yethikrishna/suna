...
reset. A v3 session keeps its runtime profile, harness, native agent, and ACP
...
A v2 session uses OpenCode REST. A v3 session uses the runtime profile selected
by its logical agent. Supported harnesses are OpenCode, Claude Code, Codex, and
Pi.

Kortix stores the runtime profile, harness, config directory, and optional
native agent when the session starts. Restart and resume keep these values.
Create a new session to select another harness.
