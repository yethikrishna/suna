...
/** One 20px indent column. Drawn, not typed: box-drawing characters break into
 *  disconnected ticks at the line-height this page uses. */
function Rail({ kind }: { kind: RailKind }): ReactNode {
  if (kind === 'blank') return <span aria-hidden className="w-5 shrink-0" />;
  return (
    <span aria-hidden className="relative w-5 shrink-0">
      ...
      />
      {kind === 'tee' || kind === 'elbow' ? (
        <span className="bg-border absolute top-1/2 left-0 h-px w-2.5" />
      ) : null}
    </span>
  );
}
      ...
      <ul className="grid min-w-max grid-cols-[auto_1fr] gap-x-8 font-mono text-[12.5px] sm:gap-x-12">
        {repo.tree.map((entry, i) => (
          <li key={entry.path} className="contents">
            ...
            <span className="text-muted-foreground/60 flex h-8 items-center text-[11px] whitespace-pre">
              {entry.note}
            </span>
          </li>
        ))}
