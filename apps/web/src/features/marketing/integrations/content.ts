 ...
 *
 * Voice rules: the `comms` skill. The canonical noun is CONNECTOR, never
 * "integration", "plugin", or "extension" — the URL is /integrations because
 * that is what people search for; the page never uses the word as a noun.
 *
 * ACCURACY GATE — every claim below traces to shipped code. Do not soften or
 * inflate these without re-reading the source:
 *  - The sandbox carries one project-scoped token (`KORTIX_EXECUTOR_TOKEN`) and
 *    zero third-party secrets. Credentials resolve server-side in the executor
 *    gateway (`apps/api/src/executor/gateway.ts`), which is the chokepoint every
 ...
 *  - Audit rows land in `kortix.executor_executions` with hashed inputs and
...
export const connect = {
  ...
  shot: {
    src: '/media/integrations/connector-catalogue.webp',
...
export const broker = {
  ...
  after: {
    ...
    lines: ['KORTIX_EXECUTOR_TOKEN=kortix_pat_…'],
  ...
  flow: [
    {
      ...
      mono: 'executor.call("gmail", "send_email", {…})',
