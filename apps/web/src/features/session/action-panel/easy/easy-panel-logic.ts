...
/** Plain-language header for one {@link groupOutputsByKind} bucket — never the
 *  raw `OutputItem['kind']` value, which is machine vocabulary ("app", not
 *  "Apps"). */
const KIND_GROUP_LABEL: Record<OutputItem['kind'], string> = {
  app: 'Apps',
...
/** Outputs at or below this count read fine as one flat list — grouping earns
 *  its keep only once a run's output is long enough that a person would
 *  otherwise have to scan the whole thing to find, say, the one video among
 *  eleven files. */
const KIND_GROUP_THRESHOLD = 10;

/** One kind's slice of a grouped output list — see {@link groupOutputsByKind}. */
export interface OutputKindGroup {
...
/**
 * Splits a long, kind-mixed output list into labeled groups ("Documents",
 * "Images", …) for the expanded Outputs list — a wall of forty rows with no
 * structure is exactly the kind of scaffolding-era output this whole card
 * exists to organize away from.
 *
 * Returns `null` — "don't group" — in the two cases where a header would say
 * nothing the flat list didn't already:
 * - the list is short enough to scan as-is ({@link KIND_GROUP_THRESHOLD});
 * - every item is the same kind, so there is only ever one group to show.
 *
 * Group order follows the FIRST appearance of each kind in `outputs`, not a
 * fixed priority table. `outputs` arrives already deliverable-first sorted
 * (`sortOutputs` in `output-priority.ts`) by the time it reaches this
 * function, so inheriting that order — rather than re-deriving a second
 * opinion about which kind matters most — is what keeps the grouped view
 * agreeing with the flat view about what leads. Item order within a group is
 * untouched input order.
 *
 * Pure: no state, no DOM, nothing beyond the `OutputItem` shape — the same
 * reason every other helper in this file is a bare function.
 */
export function groupOutputsByKind(outputs: OutputItem[]): OutputKindGroup[] | null {
  if (outputs.length <= KIND_GROUP_THRESHOLD) return null;

  const order: OutputItem['kind'][] = [];
  const buckets = new Map<OutputItem['kind'], OutputItem[]>();
  for (const output of outputs) {
    let bucket = buckets.get(output.kind);
    if (!bucket) {
      bucket = [];
      buckets.set(output.kind, bucket);
      order.push(output.kind);
    }
    bucket.push(output);
  }
  ...
  return order.map((kind) => ({ kind, label: KIND_GROUP_LABEL[kind], items: buckets.get(kind)! }));
}
