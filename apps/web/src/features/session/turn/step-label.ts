 ...
 *
 * Memory is SPLIT here, at tool granularity (W8). `get_mem` and `memory_search`
 * are lookups: the agent consulting itself, which changes nothing and is the
 * bookkeeping this set exists for. The bare `memory` tool is the memory EDITOR —
