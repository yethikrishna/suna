  ...
  const server = Bun.serve({
    ...
    async fetch(req) {
      ...
      if (resolveMatch && method === 'POST') {
        ...
        return Response.json({
          ok: true,
          scope: (body as { scope?: string })?.scope ?? 'once',
        });
      }
...
describe('approvals + wrapper project access', () => {
  ...
  test('approving posts the decision to the approval route for that execution', async () => {
    ...
    await kortix
      .project(PROJECT_ID)
      .approvals.resolve(PENDING_EXECUTION, 'approve', 'once');

    expect(upstream.requests).toHaveLength(1);
    const posted = upstream.requests[0]!;
    expect(posted.method).toBe('POST');
    expect(posted.path).toBe(
      `/v1/projects/${PROJECT_ID}/approvals/${PENDING_EXECUTION}`,
    );
  ...
  test('a standing "always this session" approval sends the session scope', async () => {
    ...
    await kortix
      .project(PROJECT_ID)
      .approvals.resolve(PENDING_EXECUTION, 'approve', 'session');
