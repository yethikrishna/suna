# Native connector authentication lifecycle implementation plan
...
- Add discovery, Authorization Code start, callback, device start, and device
  poll routes.
- Preserve existing connection ownership checks.
- Delete local credentials during revocation for every upstream result.

## Task 5: Connector and non-OAuth request authentication
...
- Run contract, API, connector, SDK, and web tests.
