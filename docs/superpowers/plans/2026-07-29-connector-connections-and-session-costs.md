# Connections and session costs implementation plan

**Spec:** `docs/superpowers/specs/2026-07-29-connector-connections-and-session-costs-design.md`

## Task 1: Connector contracts

- Add RED manifest, database, API-contract, and SDK tests.
- Add `authorization_strategy: project | user` to connectors.
- Preserve connector name, slug, provider app, strategy, and policies
  through manifest parse and serialization.
- Add connection terminology to API and SDK contracts.
- Remove connection-specific policy methods and runtime precedence.
- Apply the additive migration to the isolated database.

## Task 2: Strategy-based session connections
...
- Resolve project and user connections from connector strategy.
- Return `CONNECTOR_CONNECTION_REQUIRED` before sandbox startup.
- Enforce strategy during create, default binding, explicit binding, and
  rescope.
- Add scope read-back and `connection_id` binding terminology.

## Task 3: Main web session scope

- Read the closest session and customize reference components.
- Add component tests for missing-connection and rescope states.
- Add a compact composer control for secrets and connections.
- Render the missing-connection action.
- Use the single SDK client for read-back, replacement, and connections.
