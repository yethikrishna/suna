...
**Spec:** [Spec: sandbox mid-session stop](https://linear.app/sutharjay/document/spec-sandbox-mid-session-stop-ee52b0ddc056) — read §3 (kill paths) and §8 (constraints) before starting any task.
...
**Ticket:** JAY-440. No dependencies — start here.
...
**Ticket:** JAY-428. Depends on Task 1 (needs a runnable reaping suite).
...
applyStoppedState. Prerequisite for classifying stop paths (JAY-424)."
```
...
**Ticket:** JAY-424. Depends on Task 2 shipping to dev **and** enough elapsed time for reasons to accumulate (allow ≥ 48h of dev traffic, or query prod once Task 2 is promoted).
...
- [ ] **Step 5: Post the result as a comment on JAY-424 and close it**
...
**Ticket:** JAY-425. **Blocked by Task 3.** Ship only if Path B-waiting is material.
...
**Ticket:** JAY-426. **Blocked by Task 3.** This is the one task allowed to conclude "no safe seam exists".
...
Before writing code, answer in a Linear comment on JAY-426: **is there any request the control plane sees end-to-end that proves a turn is doing real work, which the sandbox's own credentials cannot produce?**
...
Record the finding in the Spec under §11.1b and close JAY-426 as `WON'T DO (no attested seam)`. **Do not** ship a partial signal the box can influence. A documented gap beats a rebuilt lease.

---

### Task 6: Path A′ — a reopened session must survive being read

**Ticket:** JAY-439. **Blocked by Task 3.**
...
- [ ] **Step 1: Record the decision on JAY-439 before coding**
...
**Ticket:** JAY-427. **Blocked by Task 3.** SDK package — `packages/sdk/AGENTS.md` rules apply: TDD mandatory, exported names are a public contract, never bump `version`.
...
**Ticket:** JAY-429. **Blocked by Task 7** — copy must not paper over a broken wake.
...
**Ticket:** JAY-430. **Blocked by Tasks 4–7 and Task 1.**
