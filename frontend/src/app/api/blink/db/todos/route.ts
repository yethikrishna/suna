...
export async function GET(req: Request) {
  try {
    ...
    return NextResponse.json({ error: 'Blink SDK not properly configured' }, { status: 400 })
  }
}
...
export async function DELETE(req: Request) {
  try {
    ...
    return NextResponse.json({ success: false, error: 'Blink SDK not properly configured' })
  } catch (err) {
    // const e = handleBlinkError(err)
    return NextResponse.json({ error: 'Blink SDK not properly configured' }, { status: 400 })
  }
