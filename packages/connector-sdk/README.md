# @kortix/connector-sdk

TypeScript client for the **Kortix connector gateway**. Use it when a connector workflow
...
kortix connectors discover "send a slack message"
kortix connectors show slack.send_message
kortix connectors call slack send_message '{"channel":"#general","text":"Shipped"}'
```

The SDK, the `kortix connectors` CLI, and the optional `kortix connectors mcp`
server all use the same Connector gateway. Every call runs server-side, where the
...
const tools = await connector.discover('send a slack message');
const schema = await connector.describe('slack.send_message');
const result = await connector.call('slack', 'send_message', {
  channel: '#general',
  text: 'Shipped',
});
...
const unread = await connector.call<{ messages: Array<{ id: string; from: string; text: string }> }>(
...
for (const message of unread.data?.messages ?? []) {
  ...
  const reply = await connector.call('email_email_inbox_bjgk', 'reply_message', {
...
const attachment = await connector.uploadAttachment(bytes, {
  filename: 'Investment Memo.pdf',
  contentType: 'application/pdf',
});

await connector.call('kortix_email', 'send_message', {
...
- Use `KORTIX_CLI_TOKEN` in a session sandbox or a user PAT outside a sandbox.
- Do not load provider API keys into scripts. The gateway attaches upstream
  credentials server-side.
- Handle `ok: false` envelopes and `ConnectorError` exceptions explicitly.
