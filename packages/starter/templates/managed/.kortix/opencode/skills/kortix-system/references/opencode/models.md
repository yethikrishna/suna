<!-- Mirrored from https://opencode.ai/docs/models/ — keep in sync with upstream. -->
...
If you've configured a custom provider, the `provider_id` is the key from the `provider` part of your config, and the `model_id` is the key from `provider.models`.
...
Here we're configuring global settings for two built-in models: `gpt-5` when accessed via the `openai` provider, and `claude-sonnet-4-5-20250929` when accessed via the `anthropic` provider. The built-in provider and model names can be found on [Models.dev](https://models.dev).
