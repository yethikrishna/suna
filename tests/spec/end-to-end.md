...
`CONN-9` `GET /connectors/projects/:id/pipedream/apps` and `/pipedream/sections` → admin → 200 or 501 (legacy Pipedream rollback provider not configured); sections returns stable bounded category slices and rejects non-members. `GET /connectors/connect-status` → authenticated deployment capability probe → 200 `{configured,provider,providers?}` and lists `composio` and/or `pipedream` when server keys are configured, without starting OAuth.
...
`CONN-24` Composio migration contract: `POST /connectors/projects/:id/connectors {provider:"composio",app}` creates a manifest-backed connector that stores no platform API key in config; `GET /connectors/projects/:id/connectors/:slug/config` returns `{provider:"composio",app,auth:{type:"none"}}` without secrets; `POST /connectors/projects/:id/connectors/:slug/connect` uses the server `COMPOSIO_API_KEY` to create or reuse the default project connection and returns `{provider:"composio",connectUrl,requestId?,connectionId}` when configured, or 404/501 when the provider key/adapter is absent; `POST …/connect/finalize` persists the connected account when Composio reports the expected account and otherwise returns `{provider:"composio",connected:false}` or 404/501. Wrong-tenant callers → 403 before provider lookup. A dashboard JWT on `POST /connectors/call` → 401, so failed-auth calls do not reach Composio without a connector principal.
