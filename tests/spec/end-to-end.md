...
The agent starts a live voice call bound to its session and hands out a join link — it does not join a third-party meeting. Whoever opens the link lands on a plain LiveKit client page (`/voice/:token`, public); the realtime provider connection is held SERVER-side (apps/voice-agent), so no provider credential or session authority ever reaches that page. The agent drives the call through the voice MCP, never a blocking read. Per-project bot name lives in `projects.metadata.meet`. Gated per-project by the `voice` experimental flag, checked directly by `voice_spawn` (no connector in front of it).

`VOICE-1` `PUT /projects/:id/channels/meet/name {name}` → `manage` → sets the bot's display name in the call (default "Kortix").
`VOICE-2` `POST /projects/:id/mcp/voice` — JSON-RPC. `initialize` → server info + tool capability. `tools/list` → exactly `voice_spawn`, `voice_read`, `send_prompt`, `run_command`, `voice_end`. No follow/tail/stream tool exists, by design: the agent loop is single-threaded and a blocking read wedges the whole session.
`VOICE-3` `tools/call voice_spawn {voice?}` → creates a LiveKit room bound to the session and returns `{call_id, join_url, cursor}` immediately; the call then runs in the background. `voice` is optional (defaults to a fixed voice); the returned `join_url` is a `/voice/:token` link the agent must relay to a human — there is no `meeting_url` input and no external join. The voice experimental flag being off surfaces as a tool error, not a silent no-op.
...
`VOICE-5` `tools/call send_prompt {call_id, text}` → the Kortix session speaks into a live call; a call that is not live → tool error rather than a silent no-op.
`VOICE-6` `tools/call voice_end {call_id}` → ends the call and tears down its LiveKit room; a call that is not live → tool error.
`VOICE-7` Unauthenticated MCP call → 401. The caller must resolve to a project principal with a session.
