...
`GH-9` `GET /git/:project/info/refs` · `GET …/compiled-checkout` · `POST …/git-upload-pack` · `POST …/git-receive-pack` → Git proxy and compiled checkout, git token auth (not JWT); bad/no token → 401/502.
