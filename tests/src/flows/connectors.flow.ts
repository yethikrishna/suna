...
function parseCliJson<T>(result: CliResult, action: string): T {
  ...
  try {
    ...
    throw new Error(
      `${action} returned invalid JSON: ${result.stdout}\n${result.stderr}`,
    );
  }
}
...
flow(
  ...
  async (ctx) => {
    ...
    await ctx.step(
      "legacy catalog alias preserves connector-principal auth → 401",
      async () => {
...
flow(
  ...
  async (ctx) => {
    const p = await ctx.fixtures.project();
    await ctx.step(
      "malformed identifier is rejected before connector lookup",
      async () => {
        const r = await ctx.client
          .as(ctx.P.OWNER)
          .put(
...
flow(
  "CONN-3",
  { domain: "connectors", routes: ["POST /v1/connectors/call"] },
  async (ctx) => {
...
flow(
  ...
  async (ctx) => {
    ...
    await ctx.step(
      "duplicate request leaves the original manifest entry unchanged",
      async () => {
        const r = await ctx.client
          .as(ctx.P.OWNER)
          .get("/v1/connectors/projects/:projectId/connectors/:slug/config", {
...
flow(
  ...
  async (ctx) => {
    ...
    await ctx.step(
      "pipedream category sections are bounded and stable → 200 or 501",
      async () => {
        const r = await ctx.client
          .as(ctx.P.OWNER)
          .get("/v1/connectors/projects/:projectId/pipedream/sections", {
    ...
    await ctx.step(
      "NONMEMBER cannot read pipedream category sections → 403",
      async () => {
        const r = await ctx.client
          .as(ctx.P.NONMEMBER)
          .get("/v1/connectors/projects/:projectId/pipedream/sections", {
...
flow(
  ...
  async (ctx) => {
    const p = await ctx.fixtures.project();
    await ctx.step(
      "project admin enables direct catalogue discovery",
      async () => {
        const r = await ctx.client
          .as(ctx.P.OWNER)
          .patch(
    ...
    await ctx.step("project admin browses the direct catalogue", async () => {
      ...
      const firstId = r.json<{ items?: Array<{ id?: string }> }>().items?.[0]
        ?.id;
      if (!firstId) return;
    ...
    await ctx.step(
      "NONMEMBER cannot browse or resolve catalogue records",
      async () => {
        ...
        const detail = await ctx.client
          .as(ctx.P.NONMEMBER)
          .get(
            "/v1/connectors/projects/:projectId/discover/connectors/detail",
            {
              params: { projectId: p.id },
...
flow(
  ...
  async (ctx) => {
    ...
    await ctx.step(
      "credential-mode: valid mode but unknown connector → 404",
      async () => {
        const r = await ctx.client
          .as(ctx.P.OWNER)
          .put(
    ...
    await ctx.step(
      "authorization strategy: unsupported value → 400",
      async () => {
        const r = await ctx.client
          .as(ctx.P.OWNER)
          .put(
    ...
    await ctx.step(
      "authorization strategy: valid value but unknown connector → 404",
      async () => {
        const r = await ctx.client
          .as(ctx.P.OWNER)
          .put(
    ...
    await ctx.step(
      "policies: well-formed but unknown connector → 404",
      async () => {
        const r = await ctx.client
          .as(ctx.P.OWNER)
          .put(
...
flow(
  ...
  async (ctx) => {
    ...
    await ctx.step(
      "source with no location returns an empty discovery",
      async () => {
        const r = await ctx.client
          .as(ctx.P.OWNER)
          .post(
    ...
    await ctx.step(
      "NONMEMBER cannot inspect connector authentication",
      async () => {
        const r = await ctx.client
          .as(ctx.P.NONMEMBER)
          .post(
...
flow(
  ...
  async (ctx) => {
    const p = await ctx.fixtures.project();
    await ctx.step(
      "delete credential for an unknown connector → 404",
      async () => {
        const r = await ctx.client
          .as(ctx.P.OWNER)
          .del(
...
flow(
  ...
  async (ctx) => {
    ...
    await ctx.step(
      "seed a real connector for the connection (mcp provider)",
      async () => {
    ...
    await ctx.step(
      "list connections → 200, empty before any connection exists",
      async () => {
        const r = await ctx.client
          .as(ctx.P.OWNER)
          .get("/v1/projects/:projectId/connections", {
    ...
    let connectionId = "";
    await ctx.step(
      "create (reconcile) a connection → 201 with a real shape",
      async () => {
    ...
    await ctx.step(
      "reconcile the caller-owned member connection → 201",
      async () => {
        ...
        for (
          let attempt = 0;
          attempt < 5 && flipped.statusCode === 404;
          attempt++
        ) {
    ...
    await ctx.step(
      "activate/credential/revoke on an unknown connectionId → 404",
      async () => {
        ...
        for (const op of ["activate", "credential", "revoke"] as const) {
          ...
          const r = await ctx.client
            .as(ctx.P.OWNER)
            .put(
              `/v1/projects/:projectId/connections/:connectionId/${op}`,
              body,
              {
  ...
  async (ctx) => {
    ...
    await ctx.step(
      ...
      async () => {
        ...
        if (!("provider" in status))
          throw new Error("connect-status omitted the provider key");
        const providers =
        ...
        if (
          ...
          throw new Error(
            "connect-status providers must be an array of strings",
          );
        }
    ...
    await ctx.step(
      ...
      async () => {
        ...
        if (!composioConfigured) {
          ...
          if (
            r.statusCode === 200 &&
            r.json<{ provider?: string }>().provider === "composio"
          ) {
            throw new Error(
              "toolkit catalog returned Composio while connect-status omitted it",
            );
          }
          return;
        }
    ...
    await ctx.step(
      ...
      async () => {
        ...
        if (
          body.toolkits.some(
            (item) => !item.categories?.includes("developer-tools"),
          )
        ) {
          throw new Error(
    ...
    await ctx.step(
      ...
      async () => {
        ...
        if (body.connectUrl)
          throw new Error(
            `no-auth toolkit returned an OAuth URL: ${body.connectUrl}`,
          );
    ...
    if (!composioConfigured) {
      await ctx.step(
        "finalize also fails closed when the server has no Composio key",
        async () => {
          const r = await ctx.client
            .as(ctx.P.OWNER)
            .post(
      ...
      await ctx.step(
        "wrong tenant is rejected before missing-provider lookup",
        async () => {
    ...
    await ctx.step(
      "finalize confirms the same no-auth connection identity",
      async () => {
        const r = await ctx.client.as(ctx.P.OWNER).post(
          "/v1/connectors/projects/:projectId/connectors/:slug/connect/finalize",
          {
    ...
    await ctx.step(
      ...
      async () => {
        ...
        otherConnectionId = connected.json<{ connectionId: string }>()
          .connectionId;
    ...
    await ctx.step(
      "project REST catalog exposes the connected no-auth action",
      async () => {
        const r = await ctx.client
          .as(ctx.P.OWNER)
          .get("/v1/connectors/projects/:projectId/catalog", {
        ...
        if (!connector)
          throw new Error(
            `REST catalog omitted connected Composio connector ${slug}`,
          );
        if (connector.provider !== "composio")
          throw new Error(
            `REST catalog returned provider ${connector.provider}`,
          );
    ...
    await ctx.step(
      ...
      async () => {
        ...
        if (data.logId !== data.requestId) {
          throw new Error(
            `provider logId ${data.logId} differs from requestId ${data.requestId}`,
          );
        }
      ...
      async () => {
        ...
        try {
          const listed = parseCliJson<{
            ...
            await cli.run(["connectors", "ls", "--json"], { env }),
            "kortix connectors ls",
          );
          const connector = listed.connectors.find(
            (item) => item.slug === slug,
          );
          if (!connector) throw new Error(`CLI catalog omitted ${slug}`);
          if (connector.provider !== "composio")
            throw new Error(
              `CLI catalog returned provider ${connector.provider}`,
            );
          if (!connector.actions.some((item) => item.path === action)) {
          ...
          if (called.ok !== true)
            throw new Error(
              `CLI call did not report success: ${JSON.stringify(called)}`,
            );
          if (called.data.provider !== "composio")
            throw new Error(
              `CLI call returned provider ${called.data.provider}`,
            );
          if (
            !called.data.logId ||
            called.data.logId !== called.data.requestId
          ) {
            throw new Error(
          ...
          if (called.data.logId === restLogId)
            throw new Error(
              "REST and CLI calls unexpectedly reused one provider log id",
            );
        } finally {
          cli.dispose();
        }
    ...
    await ctx.step(
      ...
      async () => {
        ...
        try {
          ...
          if (result.exitCode !== 0) {
            throw new Error(
              `MCP tools/list failed: ${result.all.slice(0, 2_000)}`,
            );
          }
          ...
          const finalize = tools.find(
            (tool) => tool.name === "finalize_connection",
          );
          if (
            add?.inputSchema?.properties?.provider?.enum?.[0] !== "composio"
          ) {
            throw new Error(
          ...
          if (
            !String(add.description).includes(
              "Composio is the default managed provider",
            )
          ) {
            throw new Error(
          ...
          if (!add?.inputSchema?.properties?.allow_legacy_pipedream) {
            throw new Error(
              "agent MCP omitted the explicit legacy Pipedream guard",
            );
          }
          if (!String(connect?.description).includes("Composio")) {
            throw new Error(
              `agent MCP connect tool omitted Composio: ${JSON.stringify(connect)}`,
            );
          }
          if (
    ...
    await ctx.step(
      "canonical audit readback contains both successful provider calls",
      async () => {
...
flow(
  ...
  async (ctx) => {
    ...
    const providers =
      ...
      (statusBody.provider ? [statusBody.provider] : []);
    if (!providers.includes("composio")) return;

    await ctx.step(
      "declare Gmail as a Composio-managed OAuth connector",
      async () => {
        const created = await ctx.client.as(ctx.P.OWNER).post(
          "/v1/connectors/projects/:projectId/connectors",
          {
    ...
    await ctx.step(
      "connect returns a fresh Composio Gmail authorization request",
      async () => {
        ...
        if (
          ...
          connectUrl.hostname !== "connect.composio.dev"
        ) {
          throw new Error(
            `unexpected Gmail Connect Link origin: ${connectUrl.origin}`,
          );
        }
        if (!body.sessionId.startsWith("trs_")) {
          throw new Error(
            `unexpected Gmail Composio session id: ${body.sessionId}`,
          );
        }
        if (!body.requestId.trim())
          throw new Error("Gmail authorization request id was empty");
      },
    );
  },
);
...
flow(
  ...
  async (ctx) => {
    ...
    await ctx.step(
      "save and read a redacted generic OAuth2 application",
      async () => {
    ...
    await ctx.step(
      "start Authorization Code with PKCE and read ready status",
      async () => {
    ...
    await ctx.step(
      "reject SSRF discovery and unavailable device endpoints",
      async () => {
...
flow(
  ...
  async (ctx) => {
    await ctx.step(
      "GET with a bogus token → 404 (invalid/unknown link)",
      async () => {
        const r = await ctx.client
          .as(ctx.P.ANON)
          .get("/v1/setup-links/connectors/:token", {
    ...
    await ctx.step(
      "POST .../start with a bogus token → 404 (invalid/unknown link)",
      async () => {
        const r = await ctx.client
          .as(ctx.P.ANON)
          .post(
...
flow(
  ...
  async (ctx) => {
    const p = await ctx.fixtures.project();
    await ctx.step(
      "missing slug → 400, or 501 when Pipedream is disabled",
      async () => {
        const r = await ctx.client
          .as(ctx.P.OWNER)
          .post(
    ...
    await ctx.step(
      "unconnected slug → 404 (or 501 if Pipedream isn't configured)",
      async () => {
        const r = await ctx.client
          .as(ctx.P.OWNER)
          .post(
