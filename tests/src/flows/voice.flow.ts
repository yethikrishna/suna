  ...
  async (ctx) => {
    ...
    await ctx.step("tools/list exposes no blocking tool", async () => {
      ...
      if (r.statusCode === 200) {
        ...
        if (
          names.length > 0 &&
          names.some((n) => /follow|tail|stream|wait/.test(n))
        ) {
          throw new Error(
            `voice MCP exposes a blocking tool: ${names.join(", ")}`,
          );
        }
      }
    });
...
flow(
  ...
  async (ctx) => {
    const p = await ctx.fixtures.sharedProject();
    await ctx.step(
      "unknown method → -32601 (or 401 without a session principal)",
      async () => {
