...
describe("ephemeral self-host preview stack", () => {
  it("routes every public surface through one origin", () => {
    ...
    for (const path of [
      ...
      expect(
        caddy.split("\n").find((line) => line.startsWith("  @api path ")),
      ).toContain(path);
    }
  ...
  it("rides out a container swap instead of 502ing through a redeploy", () => {
    ...
    expect(caddy.indexOf("(swap_tolerant) {")).toBeLessThan(
      caddy.indexOf(":8080 {"),
    );
  ...
  it("accepts either a GitHub App or a PAT as managed-git configuration", () => {
    ...
    expect(applyPreviewEnvironment(base, stack, app).testEnv).toContain(
      "KE2E_CAP_MANAGED_GIT=1",
    );
    ...
    expect(() =>
      applyPreviewEnvironment(base, stack, { MANAGED_GIT_GITHUB_OWNER: "o" }),
  ...
  it("adds preview ingress, Mailpit, direct database access, and preview-only Auth capacity", () => {
    const overlay = buildPreviewComposeOverlay(
  ...
  it("reads every allowed runtime secret from the deployment environment", () => {
    ...
    expect(
      readPreviewRuntimeSecrets({ ...source, DEV_DATABASE_URL: "forbidden" }),
    ).toEqual(
      Object.fromEntries(
        PREVIEW_RUNTIME_SECRET_ALLOWLIST.map((key) => [key, `${key}-value`]),
      ),
  ...
  it("pins exact images and configures the preview data plane", () => {
    ...
    expect(configured.runtimeEnv).toContain(
      `API_IMAGE=kortix/kortix-api:pr-${SHA}`,
    );
    expect(configured.runtimeEnv).toContain(
    ...
    expect(configured.testEnv).toContain(
      "E2E_MAILPIT_URL=https://preview.example/_mailpit",
    );
    expect(configured.testEnv).toContain(
  ...
  it("pins the PUBLIC host on the Next upstream for Server Actions", () => {
    ...
    expect(caddy).toContain(
      "X-Forwarded-Host 8080-abc.eu-west.sbx.platinum.dev",
    );
    expect(caddy).toContain("X-Forwarded-Proto https");
    const frontendBlock = caddy.slice(
      caddy.indexOf("reverse_proxy frontend:3000"),
    );
